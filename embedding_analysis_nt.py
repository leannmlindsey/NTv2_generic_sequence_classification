#!/usr/bin/env python3
"""
Embedding Analysis Script for Nucleotide Transformer v2

This script:
1. Extracts embeddings from a trained/pretrained NT-v2 model for sequences in CSV files
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Extract embeddings and perform embedding analysis for Nucleotide Transformer v2"
    )
    parser.add_argument(
        "--csv_dir",
        type=str,
...
def create_random_model(model_path: str, device: torch.device, seed: int = 42):
    """
    Create a randomly initialized model (same architecture, no pretrained weights).

    Args:
        model_path: Path to model (used to get config/architecture)
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Nucleotide Transformer v2 Embedding Analysis")
    print("=" * 60)
