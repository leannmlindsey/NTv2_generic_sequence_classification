#!/bin/bash
#
# NT-v2 LAMBDA replication — check that all STAGE 2 inference jobs finished.
#
...
#   EMBED      embedding/<variant>/embedding_analysis_results.json present (Surface D)
...
echo "============================================================"
echo "NT-v2 LAMBDA replication — inference check"
echo "============================================================"
...
for LEN in ${RUN_LENGTHS}; do
    ...
    for VARIANT in ${VARIANTS}; do
        ...
        # embedding (Surface D)
        if [ -f "${EMB_DIR}/embedding_analysis_results.json" ]; then
            echo "    EMBED    ok"
        else
            echo "    EMBED    MISSING"
        fi
